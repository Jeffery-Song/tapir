...
class ClientThread extends Thread
{
    ...
    public ClientThread(DB db, boolean dotransactions, Workload workload, int threadid, int threadcount, Properties props, int opcount, double targetperthreadperms)
    {
        ...
        _workload=workload;
        _opcount=opcount;
        // System.err.println("_opcount=" + _opcount);
        _opsdone=0;
    ...
    public void run()
    {
        ...
        try
        {
            ...
            {
                long st=System.currentTimeMillis();
                if (_props.getProperty("workload").compareTo("com.yahoo.ycsb.workloads.RangeWorkload")==0) {
                    while (_workload.isStopRequested(_workloadstate) == false) {
                        try {
                            _db.start();
...
public class Client
{
    ...
    public static String WORKLOAD_PROPERTY="workload";
  ...
  public static final String MAX_EXECUTION_TIME = "maxexecutiontime";
    ...
    public static void main(String[] args)
    {
        ...
        {
            if (props.containsKey(RANGE_SIZE)) 
            {
                // String[] rangesss = props.getProperty(RANGES, "").split(",");
                // Integer rangelength = rangesss.length();
                // Integer rangelength = rangesss.length();
                opcount=Integer.parseInt(props.getProperty(RANGE_SIZE,"0")) * props.getProperty(RANGES, "").split(",").length;
            }
            else if (props.containsKey(INSERT_COUNT_PROPERTY))
            {
                opcount=Integer.parseInt(props.getProperty(INSERT_COUNT_PROPERTY,"0"));
                // System.err.println("opcount=" + opcount);
            }
            else
            {
